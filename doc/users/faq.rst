...
.. plot::
    ...
    x = [str(xx) for xx in mx.arange(100)]  # strings
    y = mx.arange(100)
    ax[1].plot(x, y)
    ax[1].tick_params(axis='x', labelcolor='red', labelsize=14)

The solution is to convert the list of strings to numbers or
datetime objects (often ``mx.asarray(numeric_strings, dtype='float')`` or
``mx.asarray(datetime_strings, dtype='datetime64[s]')``).
