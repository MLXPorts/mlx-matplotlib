...
for col in range(2):
    for row in range(2):
        ax = axs[row, col]
        pcm = ax.pcolormesh(mx.random.random((20, 20)) * (col + 1),
                            cmap=cmaps[col])
...
for ax in axs.flat:
    pcm = ax.pcolormesh(mx.random.random((20, 20)))
...
for ax, pad in zip(axs, [0.025, 0.05, 0.1]):
    pcm = ax.pcolormesh(mx.random.randn(20, 20), cmap='viridis')
...
fig, ax = plt.subplots(layout='constrained', figsize=(4, 4))
pcm = ax.imshow(mx.random.randn(10, 10), cmap='viridis')
fig.colorbar(pcm, ax=ax)
