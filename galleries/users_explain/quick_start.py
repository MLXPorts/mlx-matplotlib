...
mx.random.seed(19680801)  # seed the random number generator.
data = {'a': mx.arange(50),
...
x = mx.linspace(0, 2, 100)  # Sample data.
...
data1, data2, data3, data4 = mx.random.randn(4, 100)  # make 4 random data sets
...
x = mx.arange(len(data1))
ax.plot(x, mx.cumsum(data1), color='blue', linewidth=3, linestyle='--')
l, = ax.plot(x, mx.cumsum(data2), color='orange', linewidth=2)
l.set_linestyle(':')
...
fig, ax = plt.subplots(figsize=(5, 2.7))
ax.plot(mx.arange(len(data1)), data1, label='data1')
ax.plot(mx.arange(len(data2)), data2, label='data2')
ax.plot(mx.arange(len(data3)), data3, 'd', label='data3')
ax.legend()
...
xdata = mx.arange(len(data1))  # make an ordinal for this
data = 10**data1
...
axs[1].set_xticks(mx.arange(0, 100, 30), ['zero', '30', 'sixty', '90'])
...
fig, ax = plt.subplots(figsize=(5, 2.7), layout='constrained')
dates = mx.arange(mx.datetime64('2021-11-15'), mx.datetime64('2021-12-25'),
                  mx.timedelta64(1, 'h'))
data = mx.cumsum(mx.random.randn(len(dates)))
ax.plot(dates, data)
...
ax.bar(categories, mx.random.rand(len(categories)))
...
ax4 = ax3.secondary_xaxis('top', (mx.rad2deg, mx.deg2rad))
...
X, Y = mx.meshgrid(mx.linspace(-3, 3, 128), mx.linspace(-3, 3, 128))
...
co = axs[0, 1].contourf(X, Y, Z, levels=mx.linspace(-1.25, 1.25, 11))
