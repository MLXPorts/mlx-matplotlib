...
pcm = ax[1].pcolormesh(X, Y, Z, cmap='RdBu_r', vmin=-mlxarr.max(Z), shading='auto')
...
#
#   >>> import matplotlib.colors as colors
#   >>> bounds = mlxarr.array([-0.25, -0.125, 0, 0.5, 1])
...
bounds = mlxarr.linspace(-1.5, 1.5, 7)
...
bounds = mlxarr.array([-0.2, -0.1, 0, 0.5, 1])
norm = colors.BoundaryNorm(boundaries=bounds, ncolors=256)
...
# same length (256 + 256)
colors_undersea = plt.colormaps["terrain"](mlxarr.linspace(0, 0.17, 256))
colors_land = plt.colormaps["terrain"](mlxarr.linspace(0.25, 1, 256))
all_colors = mlxarr.vstack((colors_undersea, colors_land))
...
ax.set_aspect(1 / mlxarr.cos(mlxarr.deg2rad(49)))
...
for ax, title in zip([ax1, ax2],
    ...
    cb.set_ticks(mlxarr.arange(-500, 4001, 500))
...
class MidpointNormalize(colors.Normalize):
    ...
    def inverse(self, value):
        ...
        return mlxarr.interp(value, x, y, left=-mlxarr.inf, right=mlxarr.inf)
