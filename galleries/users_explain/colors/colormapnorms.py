...
N = 100
X, Y = mx.mgrid[-3:3:complex(0, N), -2:2:complex(0, N)]
...
delta = 0.1
x = mx.arange(-3.0, 4.001, delta)
y = mx.arange(-4.0, 3.001, delta)
X, Y = mx.meshgrid(x, y)
Z1 = mx.exp(-X**2 - Y**2)
...
pcm = ax[1].pcolormesh(X, Y, Z, cmap='RdBu_r', vmin=-mx.max(Z), shading='auto')
...
N = 100
X, Y = mx.mgrid[0:3:complex(0, N), 0:2:complex(0, N)]
Z1 = (1 + mx.sin(Y * 10.)) * X**2
...
#
#   >>> import matplotlib.colors as colors
#   >>> bounds = mx.array([-0.25, -0.125, 0, 0.5, 1])
...
N = 100
X, Y = mx.meshgrid(mx.linspace(-3, 3, N), mx.linspace(-2, 2, N))
Z1 = mx.exp(-X**2 - Y**2)
Z2 = mx.exp(-(X - 1)**2 - (Y - 1)**2)
Z = ((Z1 - Z2) * 2)[:-1, :-1]
...
bounds = mx.array([-0.2, -0.1, 0, 0.5, 1])
norm = colors.BoundaryNorm(boundaries=bounds, ncolors=256)
...
# same length (256 + 256)
colors_undersea = plt.colormaps["terrain"](mx.linspace(0, 0.17, 256))
colors_land = plt.colormaps["terrain"](mx.linspace(0.25, 1, 256))
all_colors = mx.vstack((colors_undersea, colors_land))
...
ax.set_aspect(1 / mx.cos(mx.deg2rad(49)))
...
for ax, title in zip([ax1, ax2],
    ...
    cb.set_ticks(mx.arange(-500, 4001, 500))
...
class MidpointNormalize(colors.Normalize):
    ...
    def __call__(self, value, clip=None):
        ...
        return mx.ma.masked_array(mx.interp(value, x, y,
                                            left=-mx.inf, right=mx.inf))

    def inverse(self, value):
        y, x = [self.vmin, self.vcenter, self.vmax], [0, 0.5, 1]
        return mx.interp(value, x, y, left=-mx.inf, right=mx.inf)
