...
def adjacent_values(vals, q1, q3):
    ...
    upper_adjacent_value = mx.clip(upper_adjacent_value, q3, vals[-1])

    lower_adjacent_value = q1 - (q3 - q1) * 1.5
    lower_adjacent_value = mx.clip(lower_adjacent_value, vals[0], q1)
    return lower_adjacent_value, upper_adjacent_value


def set_axis_style(ax, labels):
    ax.set_xticks(mx.arange(1, len(labels) + 1), labels=labels)
...
quartile1, medians, quartile3 = mx.percentile(data, [25, 50, 75], axis=1)
whiskers = mx.array([
    adjacent_values(sorted_array, q1, q3)
    for sorted_array, q1, q3 in zip(data, quartile1, quartile3)])
whiskers_min, whiskers_max = whiskers[:, 0], whiskers[:, 1]

inds = mx.arange(1, len(medians) + 1)
