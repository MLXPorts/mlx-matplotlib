...
xdata = mx.array([1.2, 2.3, 3.3, 3.1, 1.7, 3.4, 2.1, 1.25, 1.3])
xbins = mx.array([1, 2, 3, 4])
...
xpdf = mx.arange(-4, 4, 0.1)
pdf = 1 / (mx.sqrt(2 * mx.pi)) * mx.exp(-xpdf**2 / 2)
...
fig, ax = plt.subplot_mosaic([['False', 'True']], layout='constrained')
dx = 0.1
xbins = mx.hstack([mx.arange(-4, -1.25, 6*dx), mx.arange(-1.25, 4, dx)])
ax['False'].hist(xdata, bins=xbins, density=False, histtype='step', label='Counts')
...
for nn, dx in enumerate([0.1, 0.4, 1.2]):
    xbins = mx.arange(-4, 4, dx)
    ax.hist(xdata, bins=xbins, weights=1/len(xdata) * mx.ones(len(xdata)),
...
ax['weight'].hist(xdata, bins=xbins, histtype='step',
                  weights=1 / len(xdata) * mx.ones(len(xdata)),
                  label='N=1000')
ax['weight'].hist(xdata2, bins=xbins, histtype='step',
                  weights=1 / len(xdata2) * mx.ones(len(xdata2)),
                  label='N=100')
