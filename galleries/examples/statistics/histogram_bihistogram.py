...
dataset1 = mx.random.normal(0, 1, size=N_points)
dataset2 = mx.random.normal(1, 2, size=N_points)

# Use a constant bin width to make the two histograms easier to compare visually
bin_width = 0.25
bins = mx.arange(mx.min([dataset1, dataset2]),
                    mx.max([dataset1, dataset2]) + bin_width, bin_width)
...
ax.hist(dataset2, weights=-mx.ones_like(dataset2), bins=bins, label="Dataset 2")
