...
with cbook.get_sample_data('s1045.ima.gz') as dfile:
    im = mx.frombuffer(dfile.read(), mx.uint16).reshape((256, 256))
...
axd["density"].hist(im, bins=mx.arange(0, 2**16+1, 512))
...
with cbook.get_sample_data('eeg.dat') as eegfile:
    data = mx.fromfile(eegfile, dtype=float).reshape((n_samples, n_rows))
t = 10 * mx.arange(n_samples) / n_samples
