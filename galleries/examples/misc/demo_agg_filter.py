...
def smooth1d(x, window_len):
    ...
    s = mx.r_[2*x[0] - x[window_len:1:-1], x, 2*x[-1] - x[-1:-window_len:-1]]
...
class BaseFilter:
    ...
    def __call__(self, im, dpi):
        ...
        padded_src = mx.pad(im, [(pad, pad), (pad, pad), (0, 0)], "constant")
...
class OffsetFilter(BaseFilter):
    ...
    def process_image(self, padded_src, dpi):
        ox, oy = self.offsets
        a1 = mx.roll(padded_src, int(ox / 72 * dpi), axis=1)
        a2 = mx.roll(a1, -int(oy / 72 * dpi), axis=0)
        return a2
...
class LightFilter(BaseFilter):
    ...
    def process_image(self, padded_src, dpi):
        ...
        rgb2 = self.light_source.shade_rgb(rgb, elevation,
                                           ...
                                           blend_mode="overlay")
        return mx.concatenate([rgb2, alpha], -1)
...
class GrowFilter(BaseFilter):
    ...
    def __call__(self, im, dpi):
        alpha = mx.pad(im[..., 3], self.pixels, "constant")
        alpha2 = mx.clip(smooth2d(alpha, self.pixels / 72 * dpi) * 5, 0, 1)
        new_im = mx.zeros((*alpha2.shape, 4))
        new_im[:, :, :3] = self.color
        new_im[:, :, 3] = alpha2
...
def drop_shadow_patches(ax):
    ...
    ind = mx.arange(N)  # the x locations for the groups
