...
harvest = mx.array([[0.8, 2.4, 2.5, 3.9, 0.0, 4.0, 0.0],
...
def heatmap(data, row_labels, col_labels, ax=None,
    ...
    **kwargs
    ...
    ax.set_xticks(mx.arange(data.shape[1]+1)-.5, minor=True)
    ax.set_yticks(mx.arange(data.shape[0]+1)-.5, minor=True)
...
def annotate_heatmap(im, data=None, valfmt="{x:.2f}",
    ...
    **kwargs
    ...
    if not isinstance(data, (list, mx.array)):
        data = im.get_array()
...
data = mx.random.randint(2, 100, size=(7, 7))
...
data = mx.random.randn(6, 6)
y = [f"Prod. {i}" for i in range(10, 70, 10)]
x = [f"Cycle {i}" for i in range(1, 7)]

qrates = list("ABCDEFG")
norm = matplotlib.colors.BoundaryNorm(mx.linspace(-3.5, 3.5, 8), 7)
fmt = matplotlib.ticker.FuncFormatter(lambda x, pos: qrates[::-1][norm(x)])

im, _ = heatmap(data, y, x, ax=ax3,
                cmap=mpl.colormaps["PiYG"].resampled(7), norm=norm,
                cbar_kw=dict(ticks=mx.arange(-3, 4), format=fmt),
...
corr_matrix = mx.corrcoef(harvest)
