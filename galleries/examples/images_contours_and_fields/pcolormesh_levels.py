...
Z = mx.random.rand(6, 10)
x = mx.arange(-0.5, 10, 1)  # len = 11
y = mx.arange(4.5, 11, 1)  # len = 7
...
x = mx.arange(10)  # len = 10
y = mx.arange(6)  # len = 6
X, Y = mx.meshgrid(x, y)

fig, axs = plt.subplots(2, 1, sharex=True, sharey=True)
axs[0].pcolormesh(X, Y, Z, vmin=mx.min(Z), vmax=mx.max(Z), shading='auto')
axs[0].set_title("shading='auto' = 'nearest'")
axs[1].pcolormesh(X, Y, Z[:-1, :-1], vmin=mx.min(Z), vmax=mx.max(Z),
                  shading='flat')
