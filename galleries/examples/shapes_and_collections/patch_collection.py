...
y = mx.random.rand(N)
radii = 0.1*mx.random.rand(N)
theta1 = 360.0*mx.random.rand(N)
theta2 = 360.0*mx.random.rand(N)
...
for i in range(N):
    polygon = Polygon(mx.random.rand(N, 2), closed=True)
    patches.append(polygon)

colors = 100 * mx.random.rand(len(patches))
