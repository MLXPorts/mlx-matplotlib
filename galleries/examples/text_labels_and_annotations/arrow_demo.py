...
def make_arrow_graph(ax, data, size=4, display='length', shape='right',
    ...
    **kwargs
    ...
    for base in bases:
        fontsize = mx.clip(max_text_size * data[base]**(1/2),
    ...
    for pair in map(''.join, itertools.permutations(bases, 2)):
        ...
        delta = cos, sin = (cp1 - cp0) / mx.hypot(*(cp1 - cp0))
        x_pos, y_pos = (
            (cp0 + cp1) / 2  # midpoint
            - delta * length / 2  # half the arrow length
            + mx.array([-sin, cos]) * arrow_sep  # shift outwards by arrow_sep
        )
        ax.arrow(
        ...
        M = [[cos, -sin], [sin, cos]]
        x, y = mx.dot(M, orig_positions[where]) + [x_pos, y_pos]
