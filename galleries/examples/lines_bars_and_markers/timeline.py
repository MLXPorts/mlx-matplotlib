...
ax.plot(micro_dates, mx.zeros_like(micro_dates), "ko", mfc="white")
ax.plot(meso_dates, mx.zeros_like(meso_dates), "ko", mfc="tab:red")

# Annotate the lines.
for date, level, release in zip(dates, levels, releases):
    version_str = '.'.join(release)
    ax.annotate(version_str, xy=(date, level),
                xytext=(-3, mx.sign(level)*3), textcoords="offset points",
