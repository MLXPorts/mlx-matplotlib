#!/usr/bin/env python
"""
Detect MLXArrayBackend usage, precision-reducing casts, and tensor conversions in Python files.

This script scans Python files to detect:
1. If MLXArrayBackend is imported or used
2. Casts that reduce precision (e.g., float() casts)
3. Unnecessary conversions of tensors to MLXArrayBackend
...
def check_ast_for_issues(file_path: str) -> Tuple[bool, List[str], List[str], List[Dict], List[Dict], List[Dict]]:
    """Use AST to check for MLXArrayBackend imports, usage, precision-reducing casts, tensor conversions, and Python operators."""
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
...
def analyze_file(file_path: str) -> Dict:
    """Analyze a file for MLXArrayBackend imports, usage, precision-reducing casts, tensor conversions, and Python operators."""
    ...
    ast_has_array_backend, ast_array_backend_imports, ast_array_backend_usages, precision_casts, tensor_conversions, python_operators = check_ast_for_issues(file_path)
...
def analyze_directory(directory: str, exclude_dirs: Optional[List[str]] = None) -> List[Dict]:
    """Analyze all Python files in a directory for MLXArrayBackend usage."""
...
def print_results(results: List[Dict], verbose: bool = False, show_array_backend: bool = True,
    ...
    if len(results) > 0:
        ...
        if show_array_backend:
            print("Files with MLXArrayBackend: 0 (0.00%)")
        if show_precision:
    ...
    if verbose:
        if show_array_backend and array_backend_files:
    ...
    for dir_path, stats in sorted(dir_summary.items()):
        ...
        has_issues = (
            (show_array_backend and stats["array_backend"] > 0) or
        ...
        if has_issues:
            print(f"{dir_path}:")
            if show_array_backend and stats["array_backend"] > 0:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Detect MLXArrayBackend usage, precision-reducing casts, tensor conversions, and Python operators in Python files.")
    ...
    show_array_backend = not (args.precision_only or args.conversion_only or args.operators_only)
    show_precision = not (args.array_backend_only or args.conversion_only or args.operators_only)
    show_conversion = not (args.array_backend_only or args.precision_only or args.operators_only)
    show_operators = not (args.array_backend_only or args.precision_only or args.conversion_only)
    
    if args.array_backend_only:
        show_array_backend = True
    ...
    if not (args.array_backend_only or args.precision_only or args.conversion_only or args.operators_only):
        show_array_backend = show_precision = show_conversion = show_operators = True
    
    print_results(results, args.verbose, show_array_backend, show_precision, show_conversion, show_operators)
    ...
    if args.array_backend_only:
        return bool(array_backend_files)
    elif args.precision_only:
        ...
        return bool(array_backend_files or precision_files or conversion_files or operator_files)
